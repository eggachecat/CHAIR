...
class EggachecatClassifier:
    def __init__(self):
        ...
        self.feature_labels = [
            *[f"v_{i}" for i in range(32)],
            # *[f"delta_{i}" for i in range(32)],
            # *[f"delta_2_{i}" for i in range(32)],
    ...
    def extract_features(self, token_score_records):
        ...
        for i in range(len(token_score_records)-1):
            abs_delta.append(token_score_records[i+1]-token_score_records[i])
        delta_std = np.std(abs_delta)

        abs_delta_degree_2 = []
        for i in range(len(token_score_records)-2):
            abs_delta.append(token_score_records[i+2]-token_score_records[i])
