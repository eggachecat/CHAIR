...
class DoLa:
    ...
    def lm_score(self, input_text1, input_text2, pmi=False, max_new_tokens=256, top_p=0.95, top_k=0, temperature=0.8, mature_layer=None, premature_layer=None, candidate_premature_layers=[], mode='baseline', verbose=True, remove_stop_words=False, relative_top=0.1, relative_top_value=-1000.0, post_softmax=True, data_collection_factory=None, **kwargs):
        with torch.no_grad():
            ...
            baseline_logits = baseline_outputs[0].squeeze(0)
            log_softmax_baseline_logits = baseline_logits.log_softmax(-1)  # logits to log probs
            # skip tokens in the prompt -- we only care about the answer
            log_softmax_baseline_logits = log_softmax_baseline_logits[prefix_ids.shape[-1] - 1: -1, :]
            # get logprobs for each token in the answer
            baseline_log_probs = log_softmax_baseline_logits[range(log_softmax_baseline_logits.shape[0]), continue_ids]
            baseline_score = baseline_log_probs.sum().item()
            ...
            token_score_records_list = np.array([
                _extract_log_probs_at_layer(i) for i in self.intervented_keys 
            ] + [baseline_log_probs.cpu().float()]).transpose()
            



        return baseline_score, token_score_records_list.tolist()