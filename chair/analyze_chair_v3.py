...
def _cross_train(n_observation, n_times):
    ...
    for _ in tqdm.tqdm(range(n_times), desc=f"Runnkng exp@{n_observation}"):
        ...
        cross_train_model_path = f"{WS_FOLDER}/saves/models/chair/cross_train/v2_{MODEL_NAME}_{n_observation}_{n_times}.pkl"
        ...
        output_path = f"{WS_FOLDER}/saves/paper/cross_join/v2_{MODEL_NAME}_{n_observation}_{n_times}.json"
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
        with open(output_path, "w") as fp:
